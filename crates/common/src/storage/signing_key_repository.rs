use std::{collections::HashMap, fmt::Debug, sync::{Arc, Mutex}, future::Future};
...
impl InMemorySigningKeyRepository {
    ...
    pub fn get_expired_key_ids(&self) -> Vec<Uuid> {
        let key_state = self.keys.lock().unwrap();
        key_state.expired_keys.iter().map(|key| key.key_id()).collect()
    }
}
...
impl SigningKeyRepository for InMemorySigningKeyRepository {
    async fn get_current_signing_key(&self) -> SigningKey {
        ...
        if key_state.current_signing_key.is_expired() {
            ...
            let expired_key = std::mem::replace(&mut key_state.current_signing_key, SigningKey::generate(self.key_lifetime));
    ...
    async fn force_key_rotation(&self) -> Result<(), String> {
        ...
        let mut existing_key = std::mem::replace(&mut key_state.current_signing_key, SigningKey::generate(self.key_lifetime));
        existing_key.expire();
...
mod tests {
    ...
    async fn test_multiple_rotations_accumulate_expired_keys() {
        ...
        for &key_id in &key_ids[..3] {
            assert!(expired_key_ids.contains(&key_id), "Key {} should be in expired keys", key_id);
        }
        ...
        for &key_id in &key_ids {
            ...
            assert!(verifying_key.is_some(), "Key {} should be available for verification", key_id);
        }
    }
    ...
    async fn test_concurrent_access() {
        ...
        for _ in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                repo_clone.get_current_signing_key().await
            });
            handles.push(handle);
        }
    ...
    async fn test_concurrent_rotation() {
        ...
        for _ in 0..5 {
            let repo_clone = repo.clone();
            let handle = tokio::spawn(async move {
                repo_clone.force_key_rotation().await
            });
            handles.push(handle);
        }
