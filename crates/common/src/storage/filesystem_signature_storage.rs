use crate::{epoch_signature::EpochSignature, storage::{SignatureStorage, SignatureStorageError}};
...
impl SignatureStorage for FilesystemSignatureStorage {
    ...
    async fn get_signature(&self, epoch: &u64) -> Result<Option<EpochSignature>, SignatureStorageError> {
        let signature_path = self.get_existing_signature_path(epoch);
    ...
    async fn set_signature(&mut self, epoch: &u64, signature: EpochSignature) -> Result<(), SignatureStorageError> {
        let epoch_dir = self.epoch_path(epoch);
        ...
        std::fs::write(&signature_path, serde_json::to_string(&signature).map_err(|e| SignatureStorageError::SerializationError(e))?)
            .map_err(|e| SignatureStorageError::IoError(e))?;
