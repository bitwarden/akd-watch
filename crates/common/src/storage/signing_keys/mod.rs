...
use crate::{
    ...
    storage::signing_keys::in_memory_signing_key_repository::InMemoryVerifyingKeyRepository,
};
...
impl SigningKeyRepository for SigningKeyStorage {
    async fn get_current_signing_key(
        &self,
    ) -> Result<crate::crypto::SigningKey, SigningKeyRepositoryError> {
        match self {
