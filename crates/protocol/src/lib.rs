//! Protocol-level types shared by the akd-watch server, clients, and any
//! downstream tooling. This crate carries only the data — wire DTOs, domain
//! types (epochs, ciphersuites, signing/verifying keys), and the
...
pub use akd_configurations::AkdConfiguration;
use chrono::Duration;
pub use epoch_signature::EpochSignedMessage;
pub use epoch_signature::{EpochSignature, EpochSignatureV1, SignError, VerifyError};
...
/// Usage:
/// ```rust,no_run
/// use akd_watch_protocol::timed_event;
///
/// async fn example() {
...
mod tests {
    ...
    async fn test_timed_event_preserves_error() {
        ...
        assert_eq!(result.expect_err("Err arm"), "test error");
    }
    ...
    async fn test_timed_event_with_complex_future() {
        ...
        assert_eq!(result.expect("Ok arm"), "Value: 42");
    }

    #[tokio::test]
    async fn test_timed_event_with_complex_future_and_fields() {
        let result =
            timed_event!(DEBUG, complex_async_function(100); operation = "database_query", query_id = 123)
                .await;
        assert_eq!(result.expect("Ok arm"), "Value: 100");
    }
    ...
    async fn test_timed_event_error_case() {
        ...
        assert_eq!(result.expect_err("Err arm"), "Invalid value");
    }
    ...
    async fn test_timed_event_result_aware_status_code() {
        let status = timed_event!(with_result(code) WARN, async { 404u16 };
